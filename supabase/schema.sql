...
-- règles (EN). Recherche moderne : préfixe (as-you-type), multi-mots dans
-- n'importe quel ordre/position, tolérance aux fautes de frappe.
...
-- Vocabulaire du corpus (mots réels du nom + texte) pour la correction de fautes.
-- À rafraîchir après chaque ingestion :
--   insert into public.search_vocab(word)
--   select word from ts_stat('select search_doc from public.cards')
--   where word ~ '^[a-z0-9]{2,}$' on conflict do nothing;
...
-- Rafraîchit le vocabulaire depuis le corpus courant (service role uniquement,
-- appelé par la fonction edge `ingest` avec ?step=vocab après ingestion).
...
-- ── Lecture publique (anon), écritures réservées au service role ────────────
...
-- Chaque mot = préfixe (as-you-type, ordre/position libres). Si le mot ressemble
-- à un mot connu (faute de frappe), on ajoute la correction en OR — jamais à la
-- place du littéral, pour ne pas dégrader un mot valide absent du vocabulaire.
...
begin
  for tok in
    select unnest(regexp_split_to_array(lower(btrim(coalesce(q, ''))), '\s+'))
  loop
    tok := regexp_replace(tok, '[^a-z0-9]', '', 'g');
    ...
    if not exists (select 1 from public.search_vocab v where v.word like tok || '%') then
      select v.word, similarity(v.word, tok)
        into corrected, sim
      from public.search_vocab v
      where v.word % tok and v.word <> tok
      order by similarity(v.word, tok) desc
      limit 1;
      if corrected is null or coalesce(sim, 0) < 0.4 then
        corrected := null;
      end if;
    end if;
...
-- ── Recherche des cartes (nom EN/FR + texte des attaques/talents/règles) ────
